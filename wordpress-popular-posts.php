	...
	class WordpressPopularPosts extends WP_Widget {
		...
		protected function _get_title_sub($p, $instance) {
			...
			if ($instance['shorten_title']['active']) {
				...
				elseif (mb_strlen($title_sub) > $instance['shorten_title']['length']) {
		...
		protected function _get_post_cat($p, $instance) {
            ...
            if ($instance['stats_tag']['category']) {
				...
				if ( "" != $post_cat ) {
					...
					if (is_mobile()&& mb_strlen($category_name) > 7) {
						$post_cat = '<a href="' . get_category_link( $category_id ) . '" class="cat-id-' . $category_id . '">' . mb_substr($category_name, 0, 7) . '...</a>';
					} else {
		...
		protected function _render_image($src, $dimension, $class, $title = "", $error = null) {
			...
			'<img class="lazy" data-original="' .$src. '" src="" ' . ( false == $this->user_settings['tools']['thumbnail']['responsive'] ? 'width="' . $dimension[0] . '" height="' . $dimension[1] . '"' : '' ) . ' title="' . esc_attr($title) . '" alt="' . esc_attr($title) . '" class="' . $class . '" />' );
		...
		protected function _get_summary($id, $instance){
			...
			if ( !empty($excerpt) ) {
				...
				if ( isset($instance['post-excerpt']['words']) && $instance['post-excerpt']['words'] ) { // by words
					...
					if ( mb_strlen($excerpt) > $instance['post-excerpt']['length'] ) {
